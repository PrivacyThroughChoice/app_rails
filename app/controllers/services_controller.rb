class ServicesController < ApplicationController
  ...
  def search
    ...
    @services = Kaminari.paginate_array((services_name + services_tags).uniq).page(params[:page])
    render :index
  end
  ...
  def tagged
    if params[:tag].present?
      @services = Service.moderated.tagged_with(params[:tag]).page(params[:page])
    else
      @services = Service.moderated.page(params[:page])
    end
    render :index
  ...
  private
    ...
    def service_params
      ...
      if user_signed_in? and (current_user.moderator? or current_user.admin?)
